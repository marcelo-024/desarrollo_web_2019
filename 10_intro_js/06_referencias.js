'use strict'

/* VALORES PRIMITIVOS (number,string y boolean) */


let a = 2
let b = a //asignación de valor 
console.log('Al crear b',a , b)

b = b * b
a = a / 2
console.log('Valor de b ', b) // 4
console.log('Valor de a ', a) // 1 



/* VALORES REFERENCIADOS (object) */


let p1 = { nombre : 'Pepe', edad: 15}

let p2 = p1//Asignación de referencia

p1.nombre = 'Juan'

console.log(p1)
console.log(p2)


let a1 = {nombre : 'Raquel', edad: 34 ,saldo : 1000}
let a2 = {nombre : 'María', edad: 30 , saldo : 1200}
let a3 = {nombre : 'Juan', edad: 31 , saldo : 1100}
/* 
function inscribirMal ( ) {

    a1.curso = 'JS'
    
   
} 

inscribirMal ()*/


//////////////////////

function inscribirEnJS (alumno) {

    if(alumno) {
         alumno.curso = 'JS'
    }


}
function inscribir (alumno , curso) {

    if(alumno) {
         alumno.curso = curso
    }


}
inscribirEnJS(a1)  // paso por REFERENCIA.
inscribirEnJS(a2)  
inscribirEnJS(a3)
inscribir(a1, 'html')
inscribir (a2, 'html')
inscribir (a3, 'JS')



let saldo = 0

function invertir (x) {

let z = Math.random()
let rendimiento = (x*z)/10
return (x + rendimiento).toFixed (2)

}

a1.saldoNew = invertir (a1.saldo)//Paso por valor
a2.saldoNew = invertir (a2.saldo)//Paso por valor
a3.saldoNew = invertir (a3.saldo)//Paso por valor


console.log (a1)
console.log (a2)
console.log (a3)

let z = invertir (500)


console.log(saldo)